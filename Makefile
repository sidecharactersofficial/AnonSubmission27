...
reproduce-table3:
	$(PY) canonical/section3_faithful_diagnostic.py --device cuda
...
reproduce-table-mixednorm:
	$(PY) canonical/run_paper_mixednorm.py --check
...
reproduce-multiseed-nslkdd:
	@set -e; for m in $(METHODS); do for s in $(NSL_SEEDS); do \
	  $(PY) canonical/eval_deepfool_k1.py --attack pgd40 --dataset nsl-kdd --method $m --seed $s --suffix .fresh; \
	done; done
	$(PY) verification/compare_exh_fresh.py --attack pgd40 --dataset nsl-kdd

reproduce-multiseed-cicids:
	@set -e; for m in $(METHODS); do for s in $(CICIDS_SEEDS); do \
	  $(PY) canonical/eval_deepfool_k1.py --attack pgd40 --dataset cicids2017 --method $m --seed $s --suffix .fresh; \
	done; done
	$(PY) verification/compare_exh_fresh.py --attack pgd40 --dataset cicids2017

reproduce-multiseed-unsw:
	@set -e; for m in $(METHODS); do for s in $(UNSW_SEEDS); do \
	  $(PY) canonical/eval_deepfool_k1.py --attack pgd40 --dataset unsw_nb15 --method $m --seed $s --suffix .fresh; \
	done; done
	$(PY) verification/compare_exh_fresh.py --attack pgd40 --dataset unsw_nb15
...
reproduce-certified:
	@set -e; for d in $(DATASETS); do for m in $(METHODS); do for s in $(NSL_SEEDS); do \
	  $(PY) canonical/run_crown_vs_ibp.py --dataset $d --method $m --seed $s; \
	done; done; done
...
reproduce-scalability:
	$(PY) canonical/eval_scalability.py --dataset unsw_nb15 --method rsc --seed 42 \
...
reproduce-jsma:
	$(PY) canonical/eval_jsma_vs_exhaustive.py --dataset unsw_nb15 --method rsc --seed 42 \
